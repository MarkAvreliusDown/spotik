...
- Построчный перевод на выбранный язык — два переключаемых провайдера (настройки →
  «Провайдер перевода»):
  ...
  - облачный [DeepL](https://www.deepl.com/pro-api) — не требует GPU, есть бесплатный
    тариф с ключом (см. инструкцию ниже)
...
2. Один из двух провайдеров перевода (можно оба сразу и переключаться в настройках):
...
Ollama (локально) или DeepL (облако, переключается в настройках), SQLite (sql.js) для
кэша переводов.
